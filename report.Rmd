...
# Prikaz rezultata
results <- data.frame(accuracies,
                      precisions,
                      recalls,
                      f_measures)

rownames(results) <- c("Scenario 1", "Scenario 2", "Scenario 3")
colnames(results) <- c("Accuracy", "Precision", "Recall", "F Measure")
...
plotting_df <- cbind(results, reg_parameter, max_iterations)
...
for(scenario in 1:3){
  ...
  for(i in 1:k_folds){
    ...
    rf <- ml_random_forest(paritioned_data$train_set,
                                  formula,
                                  num_trees = num_trees[scenario],
                                  max_depth = max_depth[scenario]
    )
    
    evaluation <- ml_evaluate(rf, dataset=paritioned_data$test_set)
...
# 
knitr::kable(results,
             label = "Prikaz rezultata Random forest",
             format = "html"
) %>%
  kableExtra::kable_styling(bootstrap_options = "bordered", full_width = FALSE, font_size = 16)
end_time <- proc.time()
...
for(scenario in 1:3){
  ...
  for(i in 1:k_folds){
    ...
    rf <- ml_gbt_classifier(paritioned_data$train_set,
                                  formula,
                                  max_depth = max_depth[scenario],
                                  step_size = step_size[scenario]
    )
...
# 
knitr::kable(results,
             label = "Prikaz rezultata Gradient boost",
             format = "html"
) %>%
  kableExtra::kable_styling(bootstrap_options = "bordered", full_width = FALSE, font_size = 16)
end_time <- proc.time()
