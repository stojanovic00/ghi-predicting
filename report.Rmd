...
# RADNA ZONA KAMENICA 10

eval <- ml_evaluate(logreg, paritioned_data$test_set)
evaluated_acc <- eval$accuracy()

pred <- ml_predict(logreg, paritioned_data$test_set)
confMatrix <- confusionMatrix(factor(pull(paritioned_data$test_set, "ghi_c")), factor(pull(pred, "ghi_c")))
predicted_acc <- confMatrix$overall[["Accuracy"]]

all(factor(pull(paritioned_data$test_set, "ghi_c")) == factor(pull(pred, "ghi_c")))
